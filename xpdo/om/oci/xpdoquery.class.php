        ...
        if ($command == 'UPDATE') {
            if (!empty($this->query['set'])) {
                ...
                while (list($setKey, $setVal) = each($this->query['set'])) {
                    ...
                    if (array_key_exists($setKey, $fieldMeta) && in_array($fieldMeta[$setKey]['dbtype'], array('timestamp', 'date'))) {
                        if ($value != null) {
                            switch ($fieldMeta[$setKey]['dbtype']) {
                                case 'timestamp' :
            ...
            if (isset ($conditions[0]) && !$this->isConditionalClause($conditions[0]) && is_array($pk) && count($conditions) == count($pk)) {
                ...
                while (list ($key, $val)= each($conditions)) {
                    if (is_int($key)) {
                        ...
                        $func = false;
                        if (isset($fieldMeta[$key]) && in_array($fieldMeta[$key]['dbtype'], array('date', 'timestamp'))) {
                            switch ($fieldMeta[$key]['dbtype']) {
                                case 'timestamp' :
                                    $func = "to_timestamp(%s, 'yyyy-mm-dd HH24:Mi:ss')";
                                    break;
                                case 'date' :
                                    $func = "to_date(%s,'yyyy-mm-dd')";
                                    break;
                            }
                        ...
                        $field= array ();
                        if ($func) {
                            $field['sql'] = "{$this->xpdo->escape($alias)}.{$this->xpdo->escape($key)} {$operator} " . sprintf($func, '?');
                        } else {
