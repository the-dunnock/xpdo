...
/**
 * Contains a derivative of the xPDOObject class for oci.
 *
 * This file contains the base persistent object classes for oci, which your
 * user-defined classes will extend when implementing an xPDO object model
 * targeted at the Oracle platform.
 *
 * @package xpdo
 * @subpackage om.oci
 */
...
/**
 * Implements extensions to the base xPDOObject class for oci.
 *
 * {@inheritdoc}
 *
 * @package xpdo
...
class xPDOObject_oci extends xPDOObject {
    public static function _save(&$obj, $cacheFlag= null) {
        if ($obj->isLazy()) {
        ...
        if (!empty ($obj->_dirty)) {
            ...
            foreach (array_keys($obj->_dirty) as $_k) {
                ...
                if (isset($obj->_fieldMeta[$_k]['generated'])) {
                    if (!$obj->_new || !isset($obj->_fields[$_k]) || empty($obj->_fields[$_k])) {
                        $pkGenerated= true;
                        continue;
                    }
                }
                ...
                if ($obj->_fieldMeta[$_k]['dbtype'] == 'timestamp') {
                    $placeholders[$_k] = "to_timestamp(:\"{$_k}\", 'yyyy-mm-dd HH24:MI:SS') ";
                } else if ($obj->_fieldMeta[$_k]['dbtype'] == 'date'){
                ...
                if ($obj->_new) {
                    ...
                    $updateSql[]= $obj->xpdo->escape($_k) . " = $placeholders[$_k]";
                }
            }
            if ($obj->_new) {
                $sql= "INSERT INTO {$obj->_table} (" . implode(', ', array_values($cols)) . ") VALUES (" . implode(', ', $placeholders) . ")";
            } else {
                if ($pk && $pkn) {
                    if (is_array($pkn)) {
                        $iteration= 0;
