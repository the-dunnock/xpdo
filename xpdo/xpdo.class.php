...
class xPDO {
    ...
    /**
     * Adds metadata information about a package, including the class map.
     *
     * @param string $pkg A package name to use when looking up classes/maps in xPDO.
     * @param string $path The root path for looking up classes in this package.
     * @return bool
    ...
    public function setPackageMeta($pkg,$path = '') {
        $set = false;
        if (is_string($pkg) && !empty($pkg)) {
            $mapFile = $path.$pkg.DIRECTORY_SEPARATOR.'metadata.inc.php';
    ...
    /**
     * Gets a list of all derivative classes for the specified user class. Will not work with xPDOObject/xPDOSimpleObject.
